...
const ReportType: React.FC<ReportTypeProps> = ({
	...
	const [showOtherTextField, setShowOtherTextField] = useState(() => {
		// Initialize based on initialType if it corresponds to "Something Else"
		return (
			initialType !== null &&
			typeOfReport[initialType - 1]?.title === 'Something Else'
		);
	});
	...
	useEffect(() => {
		updateReportData({
			typeOfReport: selectedTypeIndex,
			otherTypeOfReport: otherReportType,
		});
	}, [selectedTypeIndex, otherReportType, updateReportData]);

	const handleButtonClick = (index: number) => {
		setSelectedTypeIndex(index + 1);
		const isOther = typeOfReport[index]?.title === 'Something Else';
	...
	const handleNextClick = () => {
		if (selectedTypeIndex !== null) {
			...
			alert('Please select the type of content you are reporting.'); // Consider a less disruptive error display
		}
	};
