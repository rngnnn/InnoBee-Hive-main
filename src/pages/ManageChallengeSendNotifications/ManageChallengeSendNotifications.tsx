...
// TODO: This is a temporary type definition. Please replace it with the actual type definition from the backend.
...
const ManageChallengeNotifications = () => {
	...
	const {
		...
		setNotificationsData: (notificationsData: Notification[]) => void;
	...
	return (
		<div>
			...
			<p className="bg-pri-color text-brown-900 border border-pri-color p-3 rounded my-3 text-sm">
				Send a notification to the inboxes of challenge participants by clicking
				the 'Create new notification' button. By clicking this button, you will
				be able to choose which participants to include or exclude. You can send
				the notification right away, save it as a draft, or schedule it to be
				sent automatically at a later date. This information includes a title,
				description, a document or image, a link, and the date the event
				happened.
			</p>
			<div className="flex flex-col w-full gap-4">
				<AnimatePresence>
					{notificationsData.map((item) => (
						<SendNotificationCard key={item.uuid} item={item} />
					))}
				</AnimatePresence>
			</div>
